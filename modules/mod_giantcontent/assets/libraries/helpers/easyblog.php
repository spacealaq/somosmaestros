...
class ModGiantContentEasyBlog {
	public static function getList($params) {
		...
		$articles_eb		= $params->get('articles_eb', array());
		$categories_eb		= $params->get('categories_eb', array());
		// General
		...
		// Content filtering
		$tags_eb			= $params->get('tags_eb', array());
		...
		if (JFile::exists(JPATH_SITE.'/components/com_easyblog/easyblog.php')) {
			if ($source == 'categories' && $categories_eb) {
				if ($categories_eb[0] != '') {
					$categories_eb = (count($categories_eb) == 1) ? ' = '.$categories_eb[0].'' : 'IN ('.implode(',', $categories_eb).')';
					$query->where('a.category_id '.$categories_eb);
				}
				$exclude ? $query->where('a.id NOT IN ('.$exclude.')') : '';
			} else if ($source == 'articles' && $articles_eb) {
				$query->where('a.id IN ('.implode(',', $articles_eb).')');
			} else {
				echo JText::_('MOD_GIANTCONTENT_ERROR_SOURCE');
				return;
			}
		} else {
			echo JText::_('MOD_GIANTCONTENT_ERROR_TYPE_EASYBLOG');
			return;
		}
		...
		// Content
		$query->select('a.id, a.title, a.intro AS introtext, a.content AS maintext, a.image, a.hits');
		$query->from('#__easyblog_post AS a');

		// Category
		$query->select('a.category_id, c.title AS category_name, c.alias AS category_alias');
		$query->join('LEFT', '#__easyblog_category AS c ON c.id = a.category_id');

		// Join user
		$query->select('u.id AS user_id, u.email AS user_email, u.name AS user_name');
		$query->join('LEFT', '#__users AS u ON u.id = a.created_by');

		// Join rating
		$query->select('ROUND(AVG(r.value), 2) AS rating_average, COUNT(DISTINCT r.id) AS rating_count');
		$query->join('LEFT', '#__easyblog_ratings AS r ON r.uid = a.id');
		...
		if ($show_comments || $ordering == 'most_commented' || $ordering == 'latest_commented') {
			switch ($comments_system) {
				...
				case 'default';
					$query->select('COUNT(DISTINCT cm.id) AS comments_count, MAX(cm.created) AS comments_date');
					$query->join('LEFT', '#__easyblog_comment AS cm ON cm.post_id = a.id AND cm.published = 1');
				...
				case 'komento';
					if (JFile::exists(JPATH_SITE.'/components/com_komento/komento.php')) {
						$query->select('COUNT(DISTINCT cm.id) AS comments_count, MAX(cm.created) AS comments_date');
						$query->join('LEFT', '#__komento_comments AS cm ON cm.cid = a.id AND cm.component = "com_easyblog" AND cm.published = 1');
				...
				case 'ccomment';
					if (JFile::exists(JPATH_SITE.'/components/com_comment/comment.php')) {
						...
						$query->join('LEFT', '#__comment AS cm ON cm.contentid = a.id AND cm.component = "com_easyblog" AND cm.published = 1');
				...
				case 'jcomments';
					if (JFile::exists(JPATH_SITE.'/components/com_jcomments/jcomments.php')) {
						$query->select('COUNT(DISTINCT cm.id) AS comments_count, MAX(cm.date) AS comments_date');
						$query->join('LEFT', '#__jcomments AS cm ON cm.object_id = a.id AND cm.object_group = "com_easyblog" AND cm.published = 1');
						$comments_link = '#comments';
					} else {
				...
				case 'jacomment';
					if (JFile::exists(JPATH_SITE.'/components/com_jacomment/jacomment.php')) {
						$query->select('COUNT(cm.id) AS comments_count, MAX(cm.date) AS comments_date');
						$query->join('LEFT', '#__jacomment_items AS cm ON cm.contentid = a.id AND cm.option = "com_easyblog" AND cm.published = 1');
		...
		// Filter tags
		if ($tags_eb) {
			$query->join('INNER', '#__easyblog_post_tag AS t ON t.post_id = a.id');
			$query->where('t.tag_id IN ('.implode(',', $tags_eb).')');
		}
		...
		// Filter featured
		if ($featured) {
			$query->join('INNER', '#__easyblog_featured AS f ON f.content_id = a.id');
		}

		// Access or published
		$query->where('a.published = 1 AND c.published = 1 AND a.private = 0 AND a.issitewide = 1');
		...
		foreach ($items as $i => &$item) {
			...
			$lists[$i]->comments		= '';
			$lists[$i]->readmore		= '';
			$lists[$i]->maintext		= $item->maintext;
			$lists[$i]->introtext		= $item->introtext;
			$lists[$i]->category_id		= $item->category_id;
			$lists[$i]->category_alias	= $item->category_alias;

			// Content link
			$lists[$i]->link = EasyBlogRouter::_('index.php?option=com_easyblog&view=entry&id='.$item->id);
			...
			if ($show_image) {
				...
				if ($img_source == 'media') {
					...
					if (!count($img_tag)) {
						preg_match($pattern, $item->maintext, $img_tag);
					}
					if (count($img_tag)) {
			...
			if ($show_category) {
				...
				$lists[$i]->category_link	= EasyBlogRouter::_('index.php?option=com_easyblog&view=categories&layout=listings&id='.$item->category_id);
			...
			// Show content author
			if ($show_author) {
				$lists[$i]->author_name		= $item->user_name;
			...
			if ($show_rating) {
				if ($item->rating_count > 0) {
					...
					$lists[$i]->rating_average	= number_format($item->rating_average / 2, 2);
				} else {
					$lists[$i]->rating_count	= 0;
					$lists[$i]->rating_average	= 0.00;
				}

				$rating_current = number_format($item->rating_average) * 10;
			...
			if ($show_tags) {
				...
				$query->join('LEFT', '#__easyblog_post_tag AS tm ON tm.tag_id = t.id');
				$query->where('t.published = 1 AND tm.post_id = '.$item->id);
				...
				if ($tags) {
					foreach ($tags as $tag) {
						$link_tags = EasyBlogRouter::_('index.php?option=com_easyblog&view=tags&layout=tag&id='.$tag->id);
			...
			if ($featured_label) {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT * FROM #__easyblog_featured WHERE type = "post" AND content_id = '.$item->id);
