...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_somosmaestros&task=formacions.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'formacionList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
<?php endif;?>
		...
		<table class="table table-striped" id="formacionList">
			...
			<?php foreach ($this->items as $i => $item) :
                ...
                <?php if (isset($this->items[0]->state)): ?>
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'formacions.', $canChange, 'cb'); ?>
					</td>
                <?php endif; ?>
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_somosmaestros&task=formacion.edit&id='.(int) $item->id); ?>">
