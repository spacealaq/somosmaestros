...
if ($saveOrder)
{
	$saveOrderingUrl = 'index.php?option=com_somosmaestros&task=premios.saveOrderAjax&tmpl=component';
	JHtml::_('sortablelist.sortable', 'premioList', 'adminForm', strtolower($listDirn), $saveOrderingUrl);
}
...
<?php endif;?>
		...
		<table class="table table-striped" id="premioList">
			...
			<?php foreach ($this->items as $i => $item) :
                ...
                <?php if (isset($this->items[0]->state)): ?>
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->state, $i, 'premios.', $canChange, 'cb'); ?>
					</td>
                <?php endif; ?>
				...
				<?php endif; ?>
				<?php if ($canEdit) : ?>
					<a href="<?php echo JRoute::_('index.php?option=com_somosmaestros&task=premio.edit&id='.(int) $item->id); ?>">
					<?php echo $this->escape($item->premio); ?></a>
				<?php else : ?>
					<?php echo $this->escape($item->premio); ?>
				<?php endif; ?>
				</td>
				<td>
