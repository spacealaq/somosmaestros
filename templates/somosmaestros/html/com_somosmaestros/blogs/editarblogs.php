		...
		<div class="span12">
			<div class="row">
				...
				<div class="span10">
					<div class="row"> 
						<div class="span10">
							<form id="formCrearBlog" class="row form-crear">

								<div class="span10">
									<label for="tituloBlog">Título:</label>
									<input name="tituloBlog" id="tituloBlog" type="text" value="<?php echo $p->titulo ?>">
								</div>				
								<div class="span5">
									<label for="publicoBlog">¿Blog Público?</label>
									<select name="publicoBlog" id="publicoBlog">
								...
								<div class="span5">
									...
									<select class="chosen-choices" name="categoriaBlog" id="categoriaBlog">
										<?php
											$db = JFactory::getDbo();
											$query = 'SELECT * FROM #__somosmaestros_categorias_blog WHERE state = 1';
											$db->setQuery($query);
											$categoriaBlog = $db->loadObjectList();
											foreach ($categoriaBlog as $c) {
								...
								<div class="span10">
									<label for="previewBlog">Preview</label>
									<textarea name="previewBlog" id="previewBlog"><?php echo $p->preview ?></textarea>
								</div>			
								<div class="span5">
									<label for="fuenteBlog">Fuente:</label>
									<input name="fuenteBlog" id="fuenteBlog" type="text" value="<?php echo $p->fuente ?>">
								</div>					
								<div class="span10">
									<label for="contenidoBlog">Contenido:</label>
									 <div class="btn-toolbar" data-role="editor-toolbar" data-target="#contenidoBlog">
								      <div class="btn-group">
									...
									<div id="contenidoBlog"><?php echo $p->contenido ?></div>
								</div>
								<div class="span5">
									<label>Imagen Grande - Tamaño Recomendado: 700 x 400</label>
									<img id="imagenBlog1" class="imagenGrande">
								...
								<div class="span5">
									...
									<img id="imagenBlog2" class="imagenPequeña">
								...
								<div id="segmentacion">
									...
									<div class="span5">
										<label for="delegacionBlog">Delegación</label>
										<?php echo $p->delegacion ?>
										<select class="chosen-choices" name="delegacionBlog" id="delegacionBlog" multiple>
											<?php
									...
									<div class="span5">
										<label for="tipoBlog">Tipo de Institución: </label>
										<?php echo $p->tipo_institucion ?>
										<select class="chosen-choices" name="tipoBlog" id="tipoBlog" multiple>
											<?php
												$db = JFactory::getDbo();
									...
									<div class="span5">
										<label for="segmentoBlog">Segmentos: </label>
										<?php echo $p->segmento ?>
										<select class="chosen-choices" name="segmentoBlog" id="segmentoBlog" multiple>
											<?php
												$db = JDatabaseDriver::getInstance( SMBrujula::getConexion() );
												$query = $db->getQuery(true);
									...
									<div class="span5">
										<label for="nivelBlog">Niveles:</label>
										<?php echo $p->nivel ?>
										<select class="chosen-choices" name="nivelBlog" id="nivelBlog" multiple>
											<?php
												$db = JDatabaseDriver::getInstance( SMBrujula::getConexion() );
												$query = $db->getQuery(true);
									...
									<div class="span5">
										<label for="ciudadBlog">Ciudades:</label>
										<?php echo $p->ciudad ?>
										<select class="chosen-choices" name="ciudadBlog" id="ciudadBlog" multiple>
											<?php
												$db = JDatabaseDriver::getInstance( SMBrujula::getConexion() );
												$query = $db->getQuery(true);
									...
									<div class="span5">
										<label for="areaBlog">Áreas:</label>
										<?php echo $p->area ?>
										<select class="chosen-choices" name="areaBlog" id="areaBlog" multiple>
											<?php
												$db = JDatabaseDriver::getInstance( SMBrujula::getConexion() );
												$query = $db->getQuery(true);
									...
									<div class="span5">
										<label for="rolBlog">Rol:</label>
										<?php echo $p->rol ?>
										<select class="chosen-choices" name="rolBlog" id="rolBlog" multiple>
											<?php
												$query = $db->getQuery(true);
									...
									<div class="span5">
										<label for="proyectoBlog">Proyectos:</label>
										<?php echo $p->proyecto ?>
										<select class="chosen-choices" name="proyectoBlog" id="proyectoBlog" multiple>
											<?php
												$query = $db->getQuery(true);
								...
								<div class="span5"></div>
								<div class="span10">
					  					<div id="btn_guardarBlog" class="btn btn-primary">
											<i class="icon-bookmark icon-white"></i> Guardar Blog
										</div>
								</div>
							</form>
						</div>
...
jQuery(document).ready(function(){
	...
	var tipo = jQuery('#publicoBlog').val();
		if(tipo === '1'){
			jQuery('#segmentacion').slideUp();
			jQuery('.segmentacion').slideUp();
		}

	jQuery('#publicoBlog').change(function(){
		var tipo = jQuery('#publicoBlog').val();
		if(tipo === '1'){
			jQuery('#segmentacion').slideUp();
			jQuery('.segmentacion').slideUp();
	...
	var settings2 = {
		...
		onSuccess:function(files,data,xhr){
	       console.log(jQuery(this));
	       imagen2=data;
	       jQuery("#imagenBlog2").attr("src","<?php echo JURI::base(); ?>"+data);
	       jQuery("#imagenBlog2").fadeIn();
	    },
	    deleteCallback: function(data,pd){
		    for(var i=0;i<data.length;i++){
		        jQuery.post("<?php echo JURI::base(); ?>borrarImagen.php",{op:"delete",name:data[i],fecha:<?php echo '"'.$fecha=date(Ymd).'"'; ?>},
	...
	jQuery("#btn_guardarBlog").click(function(){
					jQuery.ajax({
						type: "POST",
						url: "index.php?option=com_functions&task=guardarBlog",
						data: { 
							id: <?php echo $idBlog; ?>,
							titulo: jQuery("#tituloBlog").val(),
							publico: jQuery("#publicoBlog").val(),
							categoria: jQuery("#categoriaBlog").val(),
							preview: jQuery("#previewBlog").val(),
							fuente: jQuery("#fuenteBlog").val(),
							contenido: jQuery("#contenidoBlog").html(),
							imagen_grande: imagen1,
							imagen_pequeña: imagen2,
							delegacion: jQuery("#delegacionBlog").val(),
							tipoInstitucion: jQuery("#tipoBlog").val(),
							segmento: jQuery("#segmentoBlog").val(),
							nivel: jQuery("#nivelBlog").val(),
							ciudad: jQuery("#ciudadBlog").val(),
							area: jQuery("#areaBlog").val(),
							rol: jQuery("#rolBlog").val(),
							proyecto: jQuery("#proyectoBlog").val()
						},
						success: function( response ) {
							if( response ){
								alert("Tu blog se ha modificado correctamente");
								location.assign("index.php?option=com_somosmaestros&view=blogs&Itemid=402");
							};
						}
					});
			});
...
function initToolbarBootstrapBindings() {
      ...
      if ("onwebkitspeechchange"  in document.createElement("input")) {
        var editorOffset = jQuery('#contenidoBlog').offset();
        jQuery('#voiceBtn').css('position','absolute').offset({top: editorOffset.top, left: editorOffset.left+jQuery('#contenidoBlog').innerWidth()-35});
      } else {
        $('#voiceBtn').hide();
      }
	};
	...
	jQuery('#contenidoBlog').wysiwyg({ fileUploadError: showErrorAlert} );
