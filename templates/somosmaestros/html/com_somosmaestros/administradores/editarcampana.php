						...
						<div class="span10">
							<form id="formCrearCampana" class="row form-crear">
								...
								<div class="span5">
									...
									<input name="nombreCampana" id="nombreCampana" type="text" value="<?php echo $p->nombre ?>">
								</div>		
								<div class="span10">
								...
								<div class="span5">
									...
									<input name="inicioCampana" id="inicioCampana" class="datepicker" value="<?php echo $p->fecha_inicio ?>">
								</div>
								<div class="span5">
									<label for="finCampana">Fecha de finalización</label>
									<input name="finCampana" id="finCampana" class="datepicker" value="<?php echo $p->fecha_fin ?>">
								</div>	
								...
								<div class="span10">
									...
									<?php 
									$query1 = $db = JFactory::getDbo();
									if($p->tipo == 'Formación'){
							            $query1 = 'SELECT id, titulo FROM #__somosmaestros_formacion WHERE id = '.$p->publicacion;
											$db->setQuery($query1);
											$publicacion = $db->loadObjectList();
							        }elseif ($p->tipo == 'Blog') {
							            $query1 = 'SELECT id, titulo FROM #__somosmaestros_blogs WHERE id = '.$p->publicacion;
											$db->setQuery($query1);
											$publicacion = $db->loadObjectList();
							        }
								...
								<div class="span5">
									...
									<input name="puntosCampana" id="puntosCampana" type="text" value="<?php echo $p->puntos ?>">
								</div>
								<div class="span5">
									<label for="metaCampana">Meta:</label>
									<input name="metaCampana" id="metaCampana" type="text" value="<?php echo $p->meta ?>">
								</div>	
								<div class="span5">
									<label for="metarCampana">Meta Reservas:</label>
									<input name="metarCampana" id="metarCampana" type="text" value="<?php echo $p->meta_reservas ?>">
								</div>					
								
								<div class="span5"></div>
								<div class="span10">
									<div id="btn_guardarCampana" class="btn btn-primary btn-publicar">
										<i class="icon-bookmark icon-white"></i> Guardar Campaña
									</div>
								</div>
							</form>
						</div>
...
jQuery(document).ready(function(){
	...
	jQuery("#btn_guardarCampana").click(function(){			
		jQuery.ajax({
			type: "POST",
			url: "index.php?option=com_functions&task=guardarCampana",
			data: {
			    id: <?php echo $idCampana; ?>,
				...
				tipoInstitucion: jQuery("#tipoCampana").val(),
				...
				publicación: jQuery('#publicacionCampana').val(),
			...
			success: function( response ) {
				if( response ){
					alert("La campaña se ha modificado correctamente");
