...
$listadoCatAgenda = $modeloCatAgenda->getItems();
				...
				<div class="span10">
					<div class="row">
						<div class="span2 offset8">
							<a href="index.php?option=com_somosmaestros&view=crearcategoriaagenda&Itemid=397" class="btn btn-primary"><i class="fa fa-plus"></i> Crear Categoría</a>
						</div>
					</div>
					<div class="row"> 
						<div class="span10">
							...
							<?php foreach ($listadoCatAgenda as $cat) {
								echo '<tr>';
								echo '<td>'.$cat->id.'</td>';
								echo '<td>'.$cat->categoria.'</td>';
								?>
								<td>
						    		<a href="<?php echo JRoute::_('index.php?option=com_somosmaestros&view=categoriasagenda&layout=editarcategoria&id='.(int) $cat->id); ?>"  class="btn btn-warning">
...
<script type="text/javascript">
		function borrarCategoria(id){
				if(confirm("¿Está seguro de borrar esta categoría?")){
					jQuery.ajax({
						type: "GET",
						url: "index.php?option=com_functions&task=eliminarCategoriaAgenda",
						data: { id: id }
