...
if (!JFactory::getUser()->authorise('core.edit.state', 'com_somosmaestros'))
{
	...
	public function getItems()
	{
		...
		foreach($items as $item){
			...
			if (isset($item->agenda) && $item->agenda != '') {
				if(is_object($item->agenda)){
					$item->agenda = JArrayHelper::fromObject($item->agenda);
				}
				$values = (is_array($item->agenda)) ? $item->agenda : explode(',',$item->agenda);
			...
			$item->agenda = !empty($textValue) ? implode(', ', $textValue) : $item->agenda;
